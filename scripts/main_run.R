...
if (length(manifest_files) == 0) stop("No manifests found in the manifests/ folder!")
...
for (m_path in manifest_files) {
  ...
  manifest <- read_csv(m_path, show_col_types = FALSE, , locale = locale(encoding = "UTF-8"))
  ...
  facts <- manifest %>%
    split(1:nrow(.)) %>%
    map_df(~extract_sut_quadrant(.x, base_path = base_data_path))
  
  # B. LOAD LOOKUPS: Load the Artisan Excel sheets for this version
  rows_lookup <- load_dimension_table(current_iso, current_ver, base_path = base_data_path, type = "Rows")
  cols_lookup <- load_dimension_table(current_iso, current_ver, base_path = base_data_path, type = "Cols")
  ...
  write_xlsx(
    national_flat, 
    path = file.path(output_dir, paste0(current_iso, "_", current_ver, "_flat_table.xlsx"))
  )
  
  # E. APPEND TO MASTER DUCKDB: The "Deduplication" logic
  # We delete existing entries for this country to ensure a clean refresh
  dbExecute(con, sprintf("DELETE FROM global_flat_table WHERE iso3 = '%s'", current_iso))
